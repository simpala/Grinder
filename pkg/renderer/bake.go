...
func (e *BakeEngine) buildBLAS(atoms []BakedAtom) ([]BLASNode, []BakedAtom) {
	...
	for _, a := range atoms {
		for i := 0; i < 3; i++ {
			if a.Pos[i] < minP[i] { minP[i] = a.Pos[i] }
	...
	for i, a := range atoms {
		...
		if diag.Y > 0 { ny = (float64(a.Pos[1]) - sceneAABB.Min.Y) / diag.Y }
		if diag.Z > 0 { nz = (float64(a.Pos[2]) - sceneAABB.Min.Z) / diag.Z }
	...
	build = func(start, end int) int32 {
		...
		for i := start + 1; i < end; i++ {
			for j := 0; j < 3; j++ {
				if sortedAtoms[i].Pos[j] < curMin[j] { curMin[j] = sortedAtoms[i].Pos[j] }
				if sortedAtoms[i].Pos[j] > curMax[j] { curMax[j] = sortedAtoms[i].Pos[j] }
			}
		}
...
func (e *BakeEngine) buildTLAS(blasInfos []blasResult) []TLASNode {
	...
	build = func(infos []blasResult) int32 {
		...
		for i := 1; i < len(infos); i++ { overallAABB = overallAABB.Expand(infos[i].aabb.Min).Expand(infos[i].aabb.Max) }
