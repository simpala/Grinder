...
func (r *Renderer) Render(bounds ScreenBounds, rng *math.XorShift32) *image.RGBA {
	...
	r.subdivide(initialAABB, bounds, img, primaryShapes, r.Shapes, rng)

	return img
}

// subdivide is the core recursive rendering function.
func (r *Renderer) subdivide(aabb math.AABB3D, bounds ScreenBounds, img *image.RGBA, primaryShapes []geometry.Shape, fullScene []geometry.Shape, rng *math.XorShift32) {
            ...
            if px >= bounds.MinX && px < bounds.MaxX && py >= bounds.MinY && py < bounds.MaxY {
                ...
                if img.RGBAAt(tileX, tileY) == r.bgColor {
                    ...
                    for i := 0; i < numSamples; i++ {
                        ...
                        sy := (float64(py) + rng.Float64()) / float64(r.Height)

                        // Fine-grind search with jittered depth
                        zSample := aabb.Min.Z + (aabb.Max.Z-aabb.Min.Z)*rng.Float64()
                        worldP := r.Camera.Project(sx, sy, zSample)

                        if hitShape.Contains(worldP) {
                            norm := hitShape.NormalAtPoint(worldP)
                            c := shading.ShadedColor(worldP, norm, r.Camera.GetEye(), r.Light, hitShape, fullScene, rng)
                            rSum += float64(c.R)
                            gSum += float64(c.G)
                            bSum += float64(c.B)
                        } else {
                            rSum += float64(r.bgColor.R)
                            gSum += float64(r.bgColor.G)
                            bSum += float64(r.bgColor.B)
                        }
                    }
                    img.Set(tileX, tileY, color.RGBA{
                        R: uint8(rSum / numSamples),
                        G: uint8(gSum / numSamples),
                        B: uint8(bSum / numSamples),
                        A: 255,
                    })
                }
            }
	...
	for zi := 0; zi < 2; zi++ {
		for xi := 0; xi < 2; xi++ {
			for yi := 0; yi < 2; yi++ {
				...
				}, bounds, img, primaryShapes, fullScene, rng)
			}
		}
	}
}
